...
void streamDevide(String pathStream)
{
  ...
  println_((String)dataStream.dataPath());
	Firebase.setStreamCallback(dataStream, streamCallbackSevice, streamTimeoutCallback);
	delay(500);
}
...
void setStateToFirebase1(bool state) {
...
void setStateToFirebase2(bool state) {
...
void setStateToFirebase3(bool state) {
  println_("Set state for D3 to firebase...");
  String token = Local.getToken();
  String pathParent = (String)PARENT_PATH + "/" + token;
  if (Firebase.setBool(dataFirebase, pathParent + (String)STATE3_PATH + "/state", state)) {
